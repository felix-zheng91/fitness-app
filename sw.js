const VERSION="20260901113942";const CACHE="fitlog-"+VERSION;const ASSETS=["index.html","./","manifest.webmanifest","icon.svg"];
...
self.addEventListener("fetch",e=>{if(e.request.method!=="GET")return;const url=new URL(e.request.url);
  ...
  if(nav){e.respondWith(fetch(e.request).then(r=>{const cp=r.clone();caches.open(CACHE).then(c=>c.put(e.request,cp));return r;}).catch(()=>caches.match("index.html")));return;}
